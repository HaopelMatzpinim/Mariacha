...
docker create --name server1 --cap-add NET_ADMIN -it ubuntu
...
docker create --name server2 --cap-add NET_ADMIN -it ubuntu
docker network connect plain-net2 server2
docker start server2


docker run -e PLAIN_MAC=eth1 -e ENCRYPTED_MAC=eth2 -d --privileged -it -e ENCRYPTED_IP=mazpin2 -e PLAIN_IP=server1 --name mazpin1 mazpin1
docker run -e PLAIN_MAC=eth1 -e ENCRYPTED_MAC=eth2  -d --privileged -it -e ENCRYPTED_IP=mazpin1 -e PLAIN_IP=server2 --name mazpin2 mazpin2
